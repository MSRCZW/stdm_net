...
with TimerBlock("Good Luck") as block:
    ...
    #vis = visdom.Visdom(env='train_loss')  # 设置环境窗口的名称,如果不设置名称就默认为main
    #vis.line(X=[0], Y=[0.], win='train_loss', opts=dict(title='train_loss'))
    init_seed(1)
    ...
    for epoch in process:
        ...
        global_step,ls,train_acc = train_net(data_loader_train, model, loss_function, optimizer, global_step, args, train_writer)
        ...
        if acc > best_accu:
            ...
            torch.save(save, args.model_saved_name + '-best.state')
        ...
        if (epoch + 1) % args.num_epoch_per_save == 0:
            print(args.model_saved_name + '-' + str(epoch) + '-' + str(global_step) + '.state')
