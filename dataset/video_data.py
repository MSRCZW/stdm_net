...
def random_rot(data_numpy, theta=0.3):
    ...
    data_numpy: C,T,V,M
    """
    data_torch = torch.from_numpy(np.float32(data_numpy))
...
def random_scale(data_numpy, theta=0.1):
    """
    data_numpy: C,T,V,M
    """
    theta = (theta, )*data_numpy.shape[0]
    data_torch = torch.from_numpy(np.float32(data_numpy))
    C, T, V, M = data_torch.shape
    data_torch = data_torch.permute(3, 1, 2, 0).contiguous()# T,3,V*M
    theta = 1 + np.random.uniform(-1,1,size=len(theta))*np.array(theta)
    data_torch = data_torch*theta
